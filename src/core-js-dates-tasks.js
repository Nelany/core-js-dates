...
function getDayName(date) {
  ...
  return new Intl.DateTimeFormat('en-US', opt).format(dateStr);
}
...
function getNextFriday(date) {
  ...
  const untilFr = day === 5 ? 7 : (5 - day + 7) % 7;
  const nextFr = new Date(date);
  nextFr.setDate(date.getDate() + untilFr);
  return nextFr;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const differenceDays = Math.floor((end - start) / milliseconds) + 1;
